        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors py-8 px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="card dark:bg-gray-800 p-6 sm:p-8 prose dark:prose-invert max-w-none shadow-sm selection:bg-blue-100 dark:selection:bg-blue-900/40">
                    { /* Dynamic terms from backend if available */ }
                    <TermsContent />
                </div>
            </div>
        </div>
...
function TermsContent() {
    ...
    useEffect(() => {
        ...
        ;(async () => {
            try {
                ...
                if (!mounted) return
                if (data?.terms) setTermsHtml(data.terms)
                if (data?.requisites) setRequisites(data.requisites)
            } catch (e) {
                // ignore
            }
        })()
    ...
    // fallback static content + dynamic requisites where available
    return (
        <>
            ...
            <ul className="space-y-1 list-none pl-0">
                <li><strong>Kompaniya nomi:</strong> {requisites?.company_name || 'Daftaron MCHJ'}</li>
                <li><strong>INN:</strong> {requisites?.inn || '312805920'}</li>
                <li><strong>Manzil:</strong> {requisites?.address || "Andijon viloyati, Baliqchi tumani, Eski markaz MFY, Maydon ko'chasi, 51-uy"}</li>
                <li><strong>Telefon:</strong> {requisites?.phone || '+998999093331'}</li>
                <li><strong>Email:</strong> {requisites?.email || 'muminovxurshidbek@gmail.com'}</li>
            </ul>
        </>
    )
}
